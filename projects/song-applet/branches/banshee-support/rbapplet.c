...
#define BANSHEE_DBUS_SERVICE "org.bansheeproject.Banshee"
#define BANSHEE_DBUS_PLAYBACK_CONTROLLER_PATH "/org/bansheeproject/Banshee/PlaybackController"
#define BANSHEE_DBUS_PLAYBACK_CONTROLLER_INTERFACE "org.bansheeproject.Banshee.PlaybackController"
#define BANSHEE_DBUS_PLAYER_ENGINE_PATH "/org/bansheeproject/Banshee/PlayerEngine"
#define BANSHEE_DBUS_PLAYER_ENGINE_INTERFACE "org.bansheeproject.Banshee.PlayerEngine"
...
typedef struct {
	...
	DBusGProxy
		...
		*banshee_player_engine_proxy, *banshee_player_controller_proxy;
} ThisApplet;

static gchar const *const service_names[2] = {
	RHYTHMBOX_DBUS_SERVICE, BANSHEE_DBUS_SERVICE
};
...
static gboolean name_has_owner(DBusGProxy *proxy)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		dbus_g_proxy_call(
				...
				G_TYPE_STRING, service_names[i], G_TYPE_INVALID,
	...
	gpointer user)
{
	...
	// HERE
	if (g_strcmp0(name, RHYTHMBOX_DBUS_SERVICE))
		return;
	...
	PanelApplet *applet, gchar const *iid, gpointer data)
{
	...
	/* rhythmbox proxies */
	DBusGProxy *rhythmbox_shell_proxy, *rhythmbox_player_proxy;
	rhythmbox_shell_proxy = dbus_g_proxy_new_for_name(
			dbus_sess_conn,
			RHYTHMBOX_DBUS_SERVICE, RHYTHMBOX_DBUS_SHELL_PATH, RHYTHMBOX_DBUS_SHELL_INTERFACE);
	g_assert(rhythmbox_shell_proxy);
	rhythmbox_player_proxy = dbus_g_proxy_new_for_name(
			dbus_sess_conn,
			RHYTHMBOX_DBUS_SERVICE, RHYTHMBOX_DBUS_PLAYER_PATH, RHYTHMBOX_DBUS_PLAYER_INTERFACE);
	g_assert(rhythmbox_player_proxy);

	/* banshee proxies */
	DBusGProxy *banshee_player_controller_proxy, *banshee_player_engine_proxy;
	banshee_player_controller_proxy = dbus_g_proxy_new_for_name(
			dbus_sess_conn,
			BANSHEE_DBUS_SERVICE,
			BANSHEE_DBUS_PLAYBACK_CONTROLLER_PATH,
			BANSHEE_DBUS_PLAYBACK_CONTROLLER_INTERFACE);
	g_assert(banshee_player_controller_proxy);
	banshee_player_engine_proxy = dbus_g_proxy_new_for_name(
			dbus_sess_conn,
			BANSHEE_DBUS_SERVICE,
			BANSHEE_DBUS_PLAYER_ENGINE_PATH,
			BANSHEE_DBUS_PLAYER_ENGINE_INTERFACE);
	g_assert(banshee_player_engine_proxy);
	...
	*(ThisApplet *)this = (ThisApplet) {
		...
		dbus_proxy,
		rhythmbox_shell_proxy, rhythmbox_player_proxy,
		banshee_player_controller_proxy, banshee_player_engine_proxy
	};
