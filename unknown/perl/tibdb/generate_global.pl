...
my $output_filename = 'global_online.html';
...
foreach (@worlds) {
	...
	my %char : shared = (name => decode_entities($1), level => $2, vocation => $3, world => $$_[0]);
...
sub print_online {
    ...
    print CACHED "<table><tr><th>Name</th><th>Level</th><th>Vocation</th><th>World</th></tr>\n";
    my $char_rank = 0;
    foreach my $char (sort {$b->{level} <=> $a->{level}} @global_online) {
    	last if $char->{level} < 100;
	print CACHED "<tr>";
	for (encode_entities(++$char_rank, $char->{name}), $char->{level}, $char->{vocation}, $char->{world}) {
