...
Public Function ControlSettingsIni(save As Boolean, ctl As Variant, iniPath As String, section As String, Optional keyExt As String = "")
    Dim str As String
    Debug.Print ctl.name & keyExt
    If save Then
        WriteToINI iniPath, section, ctl.name & keyExt, ctl
    Else
        str = ReadFromINI(iniPath, section, ctl.name & keyExt, 0)
'        ctl = str
        If TypeOf ctl Is CheckBox _
        Or TypeOf ctl Is HScrollBar _
        Or TypeOf ctl Is VScrollBar _
...
Public Function ReadFromINI( _
    ...
    Optional ByVal firstCall As Boolean = True) _
As String
    'function to return the key value of any keys inside an ini section.
    If firstCall Then If CheckIfIniKeyExists(strFilePath, strSection, strKey) = False Then WriteToINI strFilePath, strSection, strKey, strDefault
...
Public Function CheckIfIniKeyExists(ByVal filePath As String, ByVal section, ByVal keyName As String) As Boolean
    ...
    str1 = ReadFromINI(filePath, section, keyName, "A", False)
    str2 = ReadFromINI(filePath, section, keyName, "B", False)
