...
Private Sub LoadWayPath(fileLoc As String)
    ...
    While temp <> "<End List>"
        ...
        temp = getNext(FN)
    Wend
    AddStatusMessage "Way Path file loaded from " & vbCrLf & fileLoc
Cancel:
    Close FN
End Sub
...
Private Sub SaveWayPath(fileLoc As String)
    ...
    Next i
    Write #FN, "<End List>"
    AddStatusMessage "Way Path file saved to " & vbCrLf & fileLoc
Cancel:
    Close FN
End Sub
    ...
    For i = 0 To LEN_CHAR
        ...
        If ReadMem(ADR_CHAR_ONSCREEN + i * SIZE_CHAR, 1) = 1 And i <> UserPos Then
            If ReadMem(ADR_CHAR_ID + i * SIZE_CHAR + 3, 1) = &H40 Then
                ...
                End If
                If stopPlx Then
                    temp = MemToStr(ADR_CHAR_NAME + i * SIZE_CHAR, 32)
                ...
                End If
            Else
                If frmIntruder.listSafe.Contains(MemToStr(ADR_CHAR_NAME + i * SIZE_CHAR, 32)) < 0 Then stopPlx = True
            End If
        ...
        If pX = tX And pY = tY And pZ = tZ Or GetTickCount >= errorTime Then
            ...
            Select Case tempStr(0)
                ...
                Case "Rope":
                    ...
                    End If
                Case Else:
                    AddStatusMessage "Cave Bot: Not yet implemented."
                    GoTo Error
            End Select
...
Error:
    ...
    AddStatusMessage "There was an error in the cavebot. A required item might be missing."
    Valid
End Sub
